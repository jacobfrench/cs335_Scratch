...
void set_title(void)
{
	...
	XMapWindow(dpy, win);
    XStoreName(dpy, win, "Classic game gauntlet");
}
...
void physics(Game *g)
{
    ...
    else if (g->ship.pos[0] +10 >= (float)xres/2) {
        g->ship.pos[0] = ((float)xres/2)-10;
        g->ship.vel[0] = 0;
    }
    else if (g->ship.pos[1] <= 0.0) {
        g->ship.pos[1] = 0;
    }
    else if (g->ship.pos[1] +100 >= (float)yres) {
        g->ship.pos[1] = (float)yres - 100;
        g->ship.vel[1] = 0;
    }
    //
    //
